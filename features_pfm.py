...
def main():
    ...
    for index in range(args.index_start, args.index_stop+1):
        ...
        # Compute steering vectors using tdoa
        ws = sp.steering_tdoa(tdoa_t)

        # Perform phase-based frequency masking using the steering vector
        Zs = bf.pfm(Xs, ws)
        Ys_target = Zs[[0],:,:]
        Ys_interf = Zs[[1],:,:]
        ...
        # Grabbing the ideal signal directly from the clean data
        ys_ideal = ss[[0],:]
